=== MP Stacks + SocialLinks ===
Contributors: johnstonphilip
Donate link: http://mintplugins.com/
Tags: message bar, header
Requires at least: 3.5
Tested up to: 4.5
Stable tag: 1.0.1.3
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

Displays links to Social Networks in a Brick using included icon font or custom image icons

== Description ==

Displays links to Social Networks in a Brick using included icon font or custom image icons

== Installation ==

This section describes how to install the plugin and get it working.

1. Upload the 'mp-stacks-sociallinks’ folder to the `/wp-content/plugins/` directory
2. Activate the plugin through the 'Plugins' menu in WordPress
3. Build Bricks under the “Stacks and Bricks” menu.
4. Publish your bricks into a “Stack”.
5. Put Stacks on pages using the shortcode or the “Add Stack” button.

== Frequently Asked Questions ==

See full instructions at http://mintplugins.com/doc/mp-stacks

== Screenshots ==


== Changelog ==

= 1.0.1.3 = March 27, 2018
* Only show desktop icons when showhider opened

= 1.0.1.2 = July 30, 2016
* Fix for social link hover color and sizes

= 1.0.1.1 = July 5, 2016
* Further CSS specification improvements

= 1.0.1.0 = July 1, 2016
* Change margin above social links images form 14% to 0
* Increased CSS Spcificity for sociallink icons

= 1.0.0.9 = January 25, 2016
* Added Tablet and Mobile specific deisplay controls for SocialLinks

= 1.0.0.8 = September 21, 2015
* Brick Metabox controls now load using ajax.
* Admin Meta Scripts now enqueued only when needed.
* Front End Scripts now enqueued only when needed.

= 1.0.0.7 = April 8, 2015
* Icon picker change from curl to wp_remote_get

= 1.0.0.6 = March 21, 2015
* Set proper defaults for size and links per row

= 1.0.0.5 = December 19, 2014
* Re-organized Meta Options using ShowHiders
* Set default values for icon size and spacing

= 1.0.0.4 = December 9, 2014
* Fixed icon colours not being set (only hover colours worked)

= 1.0.0.3 = July 24, 2014
* Stack Template updates to allow it to work with the mp-stacks-developer plugin

= 1.0.0.2 = June 19, 2014
* Overflow hidden removed for icons. Was cutting them off in certain situations.

= 1.0.0.1 = June 8, 2014
* Various bug fixes.

= 1.0.0.0 = May 26, 2014
* Original release
